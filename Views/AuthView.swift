...
final class GokigenViewModel: ObservableObject {
    ...
    func saveCurrentEntry() {
        ...
        print(
            "💾 [GokigenViewModel] エントリを保存: originalText=\(trimmed.prefix(30))..., reformulatedText=\(reformulatedText.isEmpty ? "なし" : reformulatedText.prefix(30).description + "...")"
        )
        ...
        if let userId = currentUserId {
            Task {
                do {
                    ...
                    print(
                        "❌ [GokigenViewModel] Firestoreへの保存失敗（pending に追加）: \(error.localizedDescription)"
                    )
                }
            }
        } else {
